...
function extractEvidencePassages(
  ...
  /*
   * SOURCE-FIRST:
   * Every candidate must exist verbatim in source.content after normalization.
   * We then rank candidates by whether they directly support the finding.
   */
  const paragraphs =
    raw
      .split(/\n\s*\n/)
  ...
  /*
   * Rank ONLY by direct finding ↔ source-passage alignment.
   * The model's evidence is deliberately not used here.
   */
  const ranked =
    unique
      .map((passage) => {
