...
function semanticEvidenceMatch(
  ...
  /*
   * Never allow a solution/intervention passage to become final
   * evidence, even if it shares several words with the finding.
   */
  if (isSolutionEvidenceSentence(evidence)) {
    return {
      score: 0,
      strongMatch: false,
      problemCoverage: 0,
   ...
   * IMPORTANT:
   *
   * Evidence wording on the web is often very different from the
   * wording generated by the model.
   *
   * Example:
   *
   * Finding:
   *   "High cost of custom healthcare software"
   *
   * Source:
   *   "Epic does not publish its pricing publicly. Organizations
   *    pay separately for licensing, implementation services,
   *    hardware, training, customization and maintenance."
   *
   * The source clearly supports the cost/pricing problem even though
   * it does not repeat the complete finding sentence.
   *
   * We therefore use semantic token overlap rather than requiring
   * sentence-level wording similarity.
   */

  const problemScore =
    problem.coverage * 0.65 +
    Math.min(
      problem.exactMatches /
        Math.max(problemTokens.length, 1),
      1
    ) * 0.35;

  const evidenceClaimScore =
    modelEvidence.coverage * 0.65 +
    Math.min(
      modelEvidence.exactMatches /
        Math.max(
          modelEvidenceTokens.length,
          1
        ),
      1
    ) * 0.35;
   ...
   *
   * One generic word such as "care", "clinic", "patient", or "system"
   * is not enough to establish evidence.
   */
  ...
  const meaningfulEvidenceTokens =
    modelEvidenceTokens.filter(
      (token) => token.length >= 5
    );

  const meaningfulProblemMatches =
    meaningfulProblemTokens.filter(
      (token) =>
        getSemanticTokens(evidence).some(
          (evidenceToken) =>
            tokenMatches(
              token,
              evidenceToken
  ...
  const meaningfulEvidenceMatches =
    meaningfulEvidenceTokens.filter(
      (token) =>
        getSemanticTokens(evidence).some(
          (evidenceToken) =>
            tokenMatches(
   ...
   *
   * Accept either:
   * - two meaningful semantic matches, OR
   * - strong exact overlap.
   *
   * This prevents unrelated passages from passing because of a single
   * generic word.
   */
  const directMatch =
    (
      meaningfulProblemMatches >= 2 &&
      problem.coverage >= 0.15
    ) ||
    (
      problem.exactMatches >= 2 &&
   ...
   * Evidence bridge.
   *
   * The model's evidence is NOT trusted as final evidence.
   * It is only a locator/claim. The passage still has to exist
   * verbatim in the original Tavily source.
   */
  const evidenceBridgeMatch =
    Boolean(
      finding.evidence?.trim()
    ) &&
    meaningfulEvidenceMatches >= 2 &&
    (
      modelEvidence.coverage >= 0.12 ||
      modelEvidence.exactMatches >= 2
    );

  /*
   * Strong exact phrase support:
   *
   * If at least three semantic tokens from the model evidence are
   * represented in the source, allow the evidence bridge even when
   * the overall claim is long and coverage is diluted.
   */
  const strongEvidencePhraseMatch =
    Boolean(
      finding.evidence?.trim()
    ) &&
    meaningfulEvidenceMatches >= 3 &&
    modelEvidence.exactMatches >= 1;
  ...
  return {
    ...
    strongMatch:
      directMatch ||
      evidenceBridgeMatch ||
      strongEvidencePhraseMatch,
    problemCoverage:
      problem.coverage,
    evidenceCoverage:
      modelEvidence.coverage,
    exactMatches:
      Math.max(
        problem.exactMatches,
        modelEvidence.exactMatches
    ...
    if (sentences.length > 1) {
      for (let i = 0; i < sentences.length - 1; i++) {
        ...
        if (
          ...
          !isSolutionEvidenceSentence(sentences[i]) &&
          !isSolutionEvidenceSentence(sentences[i + 1]) &&
...
function findSource(
   ...
   * IMPORTANT:
   *
   * sourceIndex is generated by the AI and is only a hint.
   * Do not trust it blindly. The model can select the wrong index
   * when several Tavily results are supplied in one prompt.
   *
   * Rank every original Tavily source using the finding AND the
   * model evidence. The evidence is only used to locate the right
   * source; the final evidence saved by this route is still extracted
   * directly from source.content.
   */
  const findingText = normalizeText(
    `${finding.title} ${finding.description}`
  );

  const modelEvidence = normalizeText(
    finding.evidence ?? ""
  );

  const findingTokens = getTokens(
    findingText
  );

  const evidenceTokens = getTokens(
    modelEvidence
  );
  ...
  const ranked = sources
    .map((source) => {
      ...
      const findingScore =
        findingTokens.length > 0
          ? findingMatches / findingTokens.length
          : 0;

      const evidenceScore =
        evidenceTokens.length > 0
          ? evidenceMatches / evidenceTokens.length
          : 0;

      const findingSimilarity =
        findingText.length > 20
          ? similarity(
              findingText,
              sourceText
            )
      ...
      /*
       * Evidence gets the highest weight because it is normally the
       * strongest clue for locating the source selected by the model.
       */
      const score =
        findingScore * 0.30 +
        evidenceScore * 0.45 +
        findingSimilarity * 0.10 +
        evidenceSimilarity * 0.15;
  ...
  console.log(
    "Finding similarity:",
    best.findingSimilarity
  );
  console.log(
    "Evidence similarity:",
    best.evidenceSimilarity
  );
  console.log(
  ...
  /*
   * Never accept a source that has essentially no relationship to
   * either the finding or the model evidence.
   */
  if (
    best.score < 0.08 &&
    best.evidenceScore < 0.08 &&
    best.findingScore < 0.08
  ) {
    return null;
  }
