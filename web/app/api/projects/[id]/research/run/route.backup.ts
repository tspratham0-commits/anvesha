...
function extractEvidencePassages(
  ...
  /*
   * Search snippets sometimes arrive
   * as one giant block.
   */
  if (
    passages.length <= 1
...
Find genuine recurring customer problems.
...
A finding must describe a real problem.
...
Do not create solutions as findings.

Stage 1:
Customer interviews / observation.

Stage 2:
Competitor research / alternative comparison /
market-gap analysis only.

Stage 3:
Willingness-to-pay / economic validation.

Stage 4:
Pilot / experiment / smoke test /
landing-page test / manual workflow.
    ...
    messages: [
      {
        role: "system",
        content:
          "You are Anvesha Research Engine. Return only valid JSON matching the supplied schema.",
      },
      {
        role: "user",
