...
function getSemanticTokens(value: string) {
  return Array.from(
    new Set(
      getTokens(value).map(normalizeSemanticToken)
    )
  );
}
...
function tokenMatches(
  ...
  const a = normalizeSemanticToken(first);
  const b = normalizeSemanticToken(second);
  ...
  return tokensBelongToSameGroup(a, b);
}
...
function claimCoverage(
  ...
  const claimTokens = getSemanticTokens(claim);
  const evidenceTokens = getSemanticTokens(evidence);
  ...
  for (const claimToken of claimTokens) {
    if (evidenceTokens.includes(claimToken)) {
      exactMatches++;
    }

    if (
      evidenceTokens.some((evidenceToken) =>
        tokenMatches(claimToken, evidenceToken)
      )
    ) {
      matchedTokens++;
    }
  ...
  return {
    exactMatches,
    matchedTokens,
    coverage: matchedTokens / claimTokens.length,
  };
}

function semanticEvidenceMatch(
  finding: Pick<ResearchFinding, "title" | "description">,
  evidence: string
) {
  /*
   * FINAL EVIDENCE RULE
   *
   * The original source passage must support the finding itself.
   * finding.evidence is only a locator supplied by the model; it can
   * never make unrelated source text pass this gate.
   */
  if (
    !evidence.trim() ||
    isSolutionEvidenceSentence(evidence)
  ) {
    return {
      score: 0,
      strongMatch: false,
  ...
  const problemTokens =
    getSemanticTokens(problemClaim);

  const evidenceTokens =
    getSemanticTokens(evidence);
  ...
  const evidenceTokenSet =
    new Set(evidenceTokens);

  const meaningfulProblemTokens =
    problemTokens.filter(
      (token) =>
        token.length >= 5 &&
        !isGenericEvidenceToken(token)
    );

  const meaningfulMatches =
    meaningfulProblemTokens.filter(
      (token) =>
        evidenceTokens.some(
          (evidenceToken) =>
            tokenMatches(
  ...
  const meaningfulCoverage =
    meaningfulProblemTokens.length > 0
      ? meaningfulMatchCount /
        meaningfulProblemTokens.length
      : 0;

  const exactMeaningfulMatches =
    meaningfulProblemTokens.filter(
      (token) =>
  ...
  /*
   * A real problem passage normally contains at least two problem-specific
   * concepts. One generic word such as "clinic", "patient", "care", or
   * "system" is not enough.
   *
   * For short/tight findings, one exact domain phrase can be enough when the
   * title itself is directly represented in the source.
   */
  const directMatch =
    (
      meaningfulMatchCount >= 2 &&
      meaningfulCoverage >= 0.20 &&
      problem.coverage >= 0.12
    ) ||
    (
      exactMeaningfulMatches >= 2 &&
      exactCoverage >= 0.20
    ) ||
    (
      meaningfulMatchCount >= 1 &&
      exactMeaningfulMatches >= 1 &&
      meaningfulProblemTokens.length <= 4 &&
      problem.coverage >= 0.25
    );

  const score =
    Math.max(
      problem.coverage * 0.70 +
        exactCoverage * 0.30,
      meaningfulCoverage * 0.70 +
        exactCoverage * 0.30
    );

  return {
    score,
    strongMatch: directMatch,
    problemCoverage: problem.coverage,
    evidenceCoverage: 0,
    exactMatches:
      exactMeaningfulMatches,
  };
...
/*
 * Generic words are useful for locating sources but are not sufficient
 * evidence that a specific business problem exists.
 */
function isGenericEvidenceToken(
  token: string
) {
  return new Set([
    "clinic",
...
function isSolutionEvidenceSentence(
   ...
   *
   * A saved evidence passage must demonstrate the PROBLEM.
   * It must not primarily describe a product, intervention,
   * recommendation, workaround, benefit, or solution.
   *
   * This is intentionally stricter than the old matcher because
   * research sources often put the problem and the vendor's
   * solution in adjacent sentences.
   */

  const solutionPatterns = [
    /\b(?:our|its|their|the)\s+(?:platform|software|app|application|product|service|solution|tool|system)\b/,

    /\b(?:ai|cloud[-\s]?based|digital|automated|technology|software)\s+(?:solution|solutions|tool|tools|platform|platforms|system|systems|service|services)\b/,

    /\b(?:offers?|provides?|delivers?|providing|delivering)\b.{0,100}\b(?:service|solution|platform|software|tool|program|care|support|training|technology)\b/,

    /\b(?:helps?|helping|allows?|enables?|enabling|designed to|built to|created to|developed to)\b/,

    /\b(?:can|could|will|would)\s+(?:help|improve|reduce|streamline|automate|simplify|solve|address|alleviate|overcome|prevent)\b/,

    /\b(?:makes?|making)\b.{0,100}\b(?:easier|more affordable|more efficient|simpler|faster|accessible)\b/,

    /\b(?:use|using|adopt|adopting|implement|implementing|leverage|leveraging)\b.{0,100}\b(?:software|technology|platform|tool|system|app|application|service|solution|ai)\b/,

    /\b(?:launch(?:ed)?|introduc(?:ed|es)|develop(?:ed|s)|creat(?:ed|es)|built|deployed)\b.{0,120}\b(?:platform|software|app|application|product|service|solution|tool|system)\b/,

    /\b(?:customers?|clinics?|patients?|organizations?|businesses?)\s+(?:can|could|should|need to)\s+(?:use|adopt|implement|leverage)\b/,

    /\b(?:with|through)\s+(?:proper|adequate)\s+(?:training|education|support)\b.{0,100}\b(?:can|will|may|overcome|improve|reduce)\b/,

    /\b(?:barriers?|challenges?|problems?)\s+(?:can|could|may|will)\s+(?:be\s+)?(?:overcome|addressed|solved|reduced|mitigated)\b/,

    /\b(?:recommended|recommendation|best practice|approach|strategy|solution|solutions|intervention|interventions)\b/,

    /\b(?:software|platform|app|application|system|tool|service|product)\b.{0,120}\b(?:includes?|offers?|features?|provides?|supports?)\b/,

    /\b(?:includes?|offers?|provides?|features?)\b.{0,120}\b(?:online appointment|automated appointment|secure messaging|patient engagement|scheduling)\b/,

    /\b(?:features?|capabilities?)\s+(?:include|such as)\b/,

    /\b(?:available|built[- ]in|integrated)\s+(?:features?|tools?|functionality)\b/,
  ];

  return solutionPatterns.some(
...
function splitIntoSentences(
  ...
  /*
   * Web extraction can produce lowercase starts after sentence breaks.
...
function extractEvidencePassages(
   ...
   *
   * Every candidate must exist in the original source.
   * The model's evidence is only a locator and is never copied here.
  ...
  const meaningfulFindingTokens =
    findingTokens.filter(
  ...
  function relevance(
    ...
    const meaningfulCoverage =
      meaningfulFindingTokens.length > 0
        ? meaningfulMatches /
          meaningfulFindingTokens.length
        : 0;

    return {
      score:
        titleCoverage * 0.70 +
        meaningfulCoverage * 0.30,
      titleMatches,
      meaningfulMatches,
    };
  }
  ...
  function isDirectlyRelevant(
    match: ReturnType<typeof relevance>
  ) {
    return (
      match.titleMatches >= 2 ||
      (
        match.titleMatches >= 1 &&
        match.meaningfulMatches >= 2
      )
    );
  }
  ...
  for (
    ...
    /*
     * A pair is allowed only when BOTH sentences independently support
     * the finding. This prevents neighboring solution/unrelated sentences
     * from being pulled into evidence.
     */
    for (
      let i = 0;
      ...
      if (
        first.length < 40 ||
        second.length < 40 ||
        isSolutionEvidenceSentence(first) ||
        isSolutionEvidenceSentence(second)
      ) {
        continue;
      }
  ...
  /*
   * Fallback for one giant extracted block.
   */
  if (
    candidates.length === 0
  ) {
  ...
  /*
   * Deduplicate exact/near-identical source passages.
   */
  const unique =
    candidates.filter(
  ...
  for (
    ...
    /*
     * One precise source sentence is better than two weak/adjacent
     * sentences. Only add a second sentence when it is independently
     * relevant.
     */
    if (
      selected.length >= 2
...
function findSource(
  ...
  /*
   * sourceIndex and model evidence are hints only.
   *
   * The source must first match the ACTUAL problem claim. Model evidence is
   * allowed to help break ties, but it must never overpower a source that
  ...
  const ranked =
    sources
      .map((source) => {
        ...
        const meaningfulFindingMatches =
          meaningfulFindingTokens.filter(
            (token) =>
              sourceTokens.some(
                (sourceToken) =>
                  tokenMatches(
        ...
        const meaningfulFindingCoverage =
          meaningfulFindingTokens.length > 0
            ? meaningfulFindingMatches /
              meaningfulFindingTokens.length
            : 0;

        /*
         * The finding claim gets 85% of the source-selection weight.
         * Model evidence gets only 15% as a locator/tie-breaker.
         */
        const score =
          meaningfulFindingCoverage * 0.60 +
          findingCoverage.coverage * 0.25 +
          evidenceCoverage.coverage * 0.15;
  ...
  console.log(
    "Finding coverage:",
    best.findingCoverage
  );
  console.log(
    "Meaningful finding coverage:",
    best.meaningfulFindingCoverage
  );
  console.log(
    "Model-evidence coverage:",
    best.evidenceCoverage
  );
  console.log(
    "Original sourceIndex source:",
    sources[finding.sourceIndex]?.title ??
      "invalid index"
  );
  console.log(
    "=========================================\n"
  ...
  /*
   * Do not accept a source selected only because the model evidence happens
   * to occur there. The actual finding must have meaningful support.
   */
  if (
    best.meaningfulFindingCoverage < 0.15 &&
    best.findingCoverage < 0.12
  ) {
    return null;
  }
...
function mergeFindings(
  ...
  for (
    ...
    /*
     * Never concatenate evidence from duplicate findings.
     * Choose the stronger, more directly aligned source passage.
     */
    const existingEvidenceMatch =
      semanticEvidenceMatch(
        existing,
        existing.evidence
      );

    const findingEvidenceMatch =
      semanticEvidenceMatch(
        finding,
        finding.evidence
      );
    ...
    if (
      findingEvidenceMatch.score >
      existingEvidenceMatch.score
    ) {
      bestEvidence =
        finding.evidence;
    } else if (
    ...
    /*
     * Never concatenate duplicate descriptions either.
     * Keep the most precise description.
     */
