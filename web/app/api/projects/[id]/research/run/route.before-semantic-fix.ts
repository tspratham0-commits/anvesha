...
function semanticEvidenceMatch(
  ...
  const problem = claimCoverage(
    problemClaim,
    evidence
  );

  /*
   * The model's evidence field is NOT trusted as final evidence.
   * It is only used as a claim to locate/validate source wording.
   * The actual evidence saved later still comes exclusively from the
   * original Tavily source.
   */
  const modelEvidence = claimCoverage(
    finding.evidence || "",
    evidence
  );

  const findingTokens = getSemanticTokens(problemClaim);
  ...
  const problemScore =
    problem.coverage * 0.7 +
    Math.min(
      problem.exactMatches / Math.max(findingTokens.length, 1),
      1
    ) * 0.3;

  const evidenceClaimScore =
    modelEvidence.coverage * 0.7 +
    Math.min(
      modelEvidence.exactMatches /
        Math.max(
          getSemanticTokens(finding.evidence || "").length,
          1
        ),
      1
    ) * 0.3;
  ...
  /*
   * Accept when either:
   * 1. the source passage directly supports the problem, OR
   * 2. the source passage strongly matches the model's evidence claim
   *    while still being verified against the original source.
   *
   * This fixes false semantic mismatches caused by different wording
   * such as "administrative burden" vs "paperwork workload" or
   * "staff shortage" vs "difficulty retaining employees".
   */
  const directMatch =
    problemScore >= 0.30 &&
    problem.matchedTokens >= 1 &&
    (
      problem.exactMatches >= 1 ||
      problem.coverage >= 0.30
    );

  const evidenceBridgeMatch =
    Boolean(finding.evidence?.trim()) &&
    evidenceClaimScore >= 0.38 &&
    modelEvidence.matchedTokens >= 2;
  ...
  return {
    score,
    strongMatch:
      directMatch || evidenceBridgeMatch,
    problemCoverage: problem.coverage,
    evidenceCoverage: modelEvidence.coverage,
    exactMatches:
      Math.max(
