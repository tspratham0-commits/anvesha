...
function extractEvidencePassages(
  ...
  const paragraphs =
    raw
      .split(/\n\s*\n/)
      .map((text) => stripMarkdownNoise(text))
      .filter(
        (text) =>
          text.length >= 60 &&
          text.length <= 1200 &&
  ...
  // Prefer sentence-level evidence. This prevents a paragraph containing
  // both a real problem and a vendor's solution from being saved wholesale.
  for (const paragraph of paragraphs) {
    ...
    for (const sentence of sentences) {
      if (
        sentence.length >= 60 &&
        sentence.length <= 500 &&
        !isBadEvidenceSentence(sentence) &&
        !isSolutionEvidenceSentence(sentence) &&
        sentenceExistsInSource(sentence, source)
      ) {
        candidates.push(sentence);
      }
    }

    // If a paragraph has no clean individual sentence, allow a short
    // two-sentence problem passage, but never include solution sentences.
  ...
  // Search snippets sometimes arrive as one giant block. Fall back to
  // sentence grouping, still excluding solution/intervention sentences.
  ...
  const ranked = unique
    .map((passage) => ({
      passage,
      match: semanticEvidenceMatch(finding, passage),
    }))
    .filter((item) => item.match.strongMatch)
...
Your job is to identify genuine, recurring CUSTOMER PROBLEMS from the supplied web research.

Do not invent facts. Do not use general knowledge that is not supported by the supplied sources.
...
A finding must describe a real-world problem experienced by customers, users,
organizations, or a clearly defined population.
...
The "evidence" field is critical.

It MUST contain a factual statement or concise passage supported by the
...
BAD:
"MEDLIFE operates mobile clinics."

GOOD:
A concise factual statement describing the documented problem, barrier,
shortage, complaint, consequence, cost, or other fact from the source.

The evidence must answer:

"WHAT FACT IN THIS SOURCE SUPPORTS THIS PROBLEM?"
...
====================
3. SOURCE INDEX MUST MATCH THE EVIDENCE
====================

sourceIndex MUST identify the exact supplied SOURCE entry that contains the
supporting evidence.

Do not select a source merely because it discusses the general topic.

The selected source must contain information supporting the actual problem.
...
Evidence must describe the PROBLEM and its consequences.

If a source contains a paragraph like:

"Small clinics operate on tight budgets, which limits their ability to hire
administrative staff. This creates manual and time-consuming processes.
Software company X provides a platform that automates these tasks."

The evidence MUST contain only the problem facts, for example:
...
- vendor claims
- descriptions of how a product works
- calls to adopt a product
- descriptions of a company's solution
as evidence for the underlying problem.

====================
6. PROBLEM QUALITY
====================

Prefer findings with evidence of:
- recurring frequency
- meaningful severity
- economic impact
- unmet need
...
Prefer a concrete problem such as:
"Small clinics face financial constraints that limit their ability to hire
administrative staff, resulting in manual and time-consuming processes."

====================
7. SCORES
====================

Scores must reflect the evidence actually found in the supplied sources.
...
economicImpact:
How much financial or operational impact the problem creates.

evidenceStrength:
How strongly the selected source supports the problem.

paymentPotential:
How likely the affected customer or organization could reasonably pay to
solve the problem.

Use conservative scores when evidence is weak.

Do NOT invent a numerical statistic merely to justify a score.

Do NOT automatically set all scores to zero when the source provides meaningful
...
- Measure how frequently the problem occurs.
- Compare existing alternatives.
- Investigate current spending.
...
Stage 2:
Competitor research / alternative comparison / market-gap analysis only.

Stage 3:
Willingness-to-pay / economic validation.

Stage 4:
Pilot / experiment / smoke test / landing-page test / manual workflow.
...
"Build an application" is NOT a validation action.

====================
10. RESEARCH STANDARD
====================

Only return a finding when the supplied source contains meaningful evidence
for the problem.

If a source only describes a company's solution, do NOT use that solution as
evidence.

If the proposed evidence is only a source name, organization name, URL,
citation, product, service, or solution, DO NOT use it.

If evidence is insufficient, choose a different problem with stronger
...
- evidence = factual source-backed problem evidence
- sourceIndex = exact source containing that evidence
- scores = conservative evidence-based scores
- actions = validation activities only

Before returning each finding, mentally verify:

A. Is this a PROBLEM?
B. Does the selected source actually support it?
C. Is the evidence a FACT rather than a source name?
D. Does the evidence describe the problem rather than the solution?
E. Does sourceIndex point to the correct source?
F. Are the actions validation activities rather than product-building tasks?

If any answer is NO, replace the finding with a better supported finding or
omit it.
